...
import { startPushServer, appClients, broadcastToAppClients } from './push-server';
...
openclawClient.onUnmatchedMessage = (text) => {
  console.log(`[OpenClaw] Unmatched bot message — broadcasting to ${activeSessions.size} sessions + ${appClients.size} app clients`);
  for (const [, h] of activeSessions) { h.display.showNotification(text); }
  broadcastToAppClients({ type: 'ai_response', text });
};
